...
-export([clear/1, delete/1, update/1, lookup/1, invalidate_sessions/0, test/0]).
...
get_value(Session, Key, Default) ->
    io:format("n2o.session: get_value: ~p~n",[{Session, Key, Default}]),
...
test() -> [test_set_get_value(),negative_test(),positive_test()].
