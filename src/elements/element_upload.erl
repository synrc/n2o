...
init([#upload_state{root=Root, dir=Dir, name=Name}=S]) ->
  case file:open(filename:join([Root, Dir, Name]), [raw, binary, write]) of
    {ok, D} -> file:close(D),{ok,S};{error, enoent}->{ok,S};{error,enotdir}->{ok,S};{error,R}->{stop,R} end.
...
terminate(normal, S) ->
  ...
  file:write_file(File, S#upload_state.data, [write, raw]),

  ?WS_SEND(S#upload_state.cid, done, []), wf:flush(S#upload_state.room);
terminate(R,S) ->
...
wire(#upload{id=Id, state=S}=R) ->
  ...
  [wf:wire(#event{postback={Id,T,Arg}, source=Src, target=Id, type=T, delegate=?MODULE}) || {T, Src, Arg} <- Callbacks].


event({_,_,#upload_state{delegate=M}}=E) when M /=undefined -> M:event(E);
...
event({Id, start_upload, #upload_state{}=S}) ->
  ...
  [{file_name, Name}, {type, MimeType}] = wf:q({Id, <<"detail">>}),
  Absname = filename:join([S#upload_state.root, S#upload_state.dir, wf:to_list(Name)]),
  {Ev, Detail} = case gen_server:start(?MODULE, [S#upload_state{cid=Id, name=wf:to_list(Name), type=MimeType}], []) of
...
event({Id, deliver, #upload_state{}=S}) ->
