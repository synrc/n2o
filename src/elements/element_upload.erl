...
-compile(export_all).
-record(state, {ctl_id, name, mimetype, data= <<>>, root="", dir="", delegate, room, post_write, img_tool, width=200, height=200}).
...
handle_cast(complete, S) ->
  ...
  case S#state.delegate of
    ...
    Module -> Module:control_event(S#state.ctl_id,
      {S#state.root, S#state.dir, S#state.name, S#state.mimetype, Data, S#state.room, S#state.post_write, S#state.img_tool})
  end,
  {stop, normal, #state{}}.
...
wire(#upload{id=Id} = R) ->
  ...
  wf:wire( wf:f("$(function(){ $('#~s').upload({"
    ...
    "complete: function(msg){~s} }); });", [Id, atom_to_list(R#upload.preview)]++ [wf_event:generate_postback_script(Tag, ignore, Id, element_upload, control_event, <<"{'msg': msg}">>)
      || Tag <- [{begin_upload, R#upload.root, R#upload.dir, R#upload.delegate, R#upload.post_write, R#upload.img_tool}, deliver_slice, {query_file, R#upload.root, R#upload.dir, R#upload.delegate_query}, complete]])).
...
control_event(Cid, Tag) ->
  ...
  {Event, Param} = case Msg of
    ...
    {begin_upload, Name, MimeType} ->
      {begin_upload, Root, Dir, Delegate, Post, Tool} = Tag,
      case gen_server:start(?MODULE, [#state{ctl_id=Cid, root=Root, dir=Dir, name=binary_to_list(Name), mimetype=MimeType, delegate=Delegate, room=Room, post_write=Post, img_tool=Tool}], []) of
