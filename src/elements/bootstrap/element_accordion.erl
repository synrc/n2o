  ...
  if R#accordion.nav_stacked == true ->
    ...
    L = #list{id=Id, class=[nav, "nav-stacked" | R#accordion.class], body=[
      begin
        Tid = wf:temp_id(),
        #li{body=[
          #link{body=Head, data_fields=[{<<"data-toggle">>, <<"collapse">>}, {<<"data-target">>, list_to_binary("#"++Tid)}], url= <<"javascript:void(0)">>},
      ...
      begin
        ...
        #panel{class=["accordion-group"], body=[
          #panel{class=["accordion-heading"], body=[
            #link{class=["accordion-toggle"], body=Head, data_fields=[{<<"data-toggle">>, <<"collapse">>}, {<<"data-parent">>, list_to_binary(Id)}], url=list_to_binary("#"++Tid)}
          ]},
