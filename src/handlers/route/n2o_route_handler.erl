...
find_optimal_path(Parts, PathInfo) ->
  case file_exists(Parts) of 
    ...
    Module -> {list_to_atom(Module), PathInfo}  % is list_to_atom ok here?
  end.
  ...
  case erlang:function_exported(Handler, main, 0) of
    ...
    false -> 
      ...
      case erlang:function_exported(?DEFAULT_HANDLER, main, 0) of
        ...
        false -> error(invalid_n2o_handler, [Handler])
      end
  end.

path_to_parts(Path) ->
  re:split(Path, "/", [{return, list}, trim]).

parts_to_path(Parts) ->
  string:join(Parts, "/").

parts_to_filename(Parts) ->
  string:join(Parts, "_").
