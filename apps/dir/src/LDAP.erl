%% Generated by the Erlang ASN.1 BER-compiler version:1.6.14
%% Purpose: encoder and decoder to the types in mod LDAP

-module('LDAP').
-include("LDAP.hrl").
-define('RT_BER',asn1rt_ber_bin).
-asn1_info([{vsn,'1.6.14'},
            {module,'LDAP'},
            {options,[{i,[46]}]}]).
...
decode_disp('PartialAttribute',Data) -> 'dec_PartialAttribute'(Data,mandatory);
...
decode_disp('URI',Data) -> 'dec_URI'(Data,mandatory);
...
decode_disp('AttributeSelection',Data) -> 'dec_AttributeSelection'(Data,mandatory);
...
decode_disp('IntermediateResponse',Data) -> 'dec_IntermediateResponse'(Data,mandatory);
...
%%-------------------------------------------------
%% attribute number 3   External LDAP:Controls OPTIONAL
%%-------------------------------------------------
...
'enc_LDAPMessage_protocolOp'(Val, TagIn) ->
      ...
      intermediateResponse ->
         'enc_IntermediateResponse'(element(2,Val), []);
      Else -> 
...
%% 'intermediateResponse'
      <<1:2,_:1,25:5,_/binary>> ->
         {Dec, Rest, RbCho} = 'dec_IntermediateResponse'(Bytes1, mandatory, []),
         {{intermediateResponse, Dec}, IndefEndBytes(Len,Rest), RbExp + RbCho + IndefEndRb(Len,Rest)};
...
%%-------------------------------------------------
   {EncBytes2,EncLen2} = 'enc_PartialAttribute_vals'(?RT_BER:cindex(3,Val,vals), []),
...
'enc_PartialAttribute_vals'({'PartialAttribute_vals',Val}, TagIn) ->
...
'dec_PartialAttribute_vals'(Bytes, OptOrMand, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
...
'dec_PartialAttribute'(Bytes, OptOrMand) ->
   'dec_PartialAttribute'(Bytes, OptOrMand, []).

'dec_PartialAttribute'(Bytes, OptOrMand, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
   {{_,Len},Bytes1,Rb1} = ?RT_BER:check_tags(TagIn ++ [{tag,0,16,'IMPLICIT',32}], Bytes, OptOrMand), 
...
%% attribute number 2 with type SET OF
%%-------------------------------------------------
   {Term2,Bytes4,Rb3} = 'dec_PartialAttribute_vals'(Bytes3, mandatory, []),

   {Bytes5,Rb4} = ?RT_BER:restbytes2(RemBytes, Bytes4,noext),
   {{'PartialAttribute', Term1, Term2}, Bytes5, Rb1+Rb2+Rb3+Rb4}.
...
'enc_Attribute'({'Attribute',Val}, TagIn) ->
...
strongerAuthRequired -> ?RT_BER:encode_enumerated(8,[]);
...
%%-------------------------------------------------
   {EncBytes3,EncLen3} = ?RT_BER:encode_octet_string([], ?RT_BER:cindex(4,Val,diagnosticMessage), []),

%%-------------------------------------------------
%% attribute number 4   External LDAP:Referral OPTIONAL
%%-------------------------------------------------
...
'dec_URI'(Bytes, OptOrMand) ->
   'dec_URI'(Bytes, OptOrMand, []).

'dec_URI'(Bytes, OptOrMand, TagIn) ->
?RT_BER:decode_octet_string(Bytes,[],TagIn++[], no_length, OptOrMand).
...
%%-------------------------------------------------
%% attribute number 3   External LDAP:AuthenticationChoice
%%-------------------------------------------------
...
%%-------------------------------------------------
%% attribute number 7   External LDAP:Filter
%%-------------------------------------------------
   {EncBytes7,EncLen7} = 'enc_Filter'(?RT_BER:cindex(8,Val,filter), []),

%%-------------------------------------------------
%% attribute number 8   External LDAP:AttributeSelection
%%-------------------------------------------------
   {EncBytes8,EncLen8} = 'enc_AttributeSelection'(?RT_BER:cindex(9,Val,attributes), []),
...
%% attribute number 8   External LDAP:AttributeSelection
%%-------------------------------------------------
   {Term8,Bytes10,Rb9} = 'dec_AttributeSelection'(Bytes9, mandatory, []),
...
'enc_AttributeSelection'({'AttributeSelection',Val}, TagIn) ->
...
'dec_AttributeSelection'(Bytes, OptOrMand) ->
   'dec_AttributeSelection'(Bytes, OptOrMand, []).

'dec_AttributeSelection'(Bytes, OptOrMand, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
   {{_,Len},Bytes1,Rb1} = ?RT_BER:check_tags(TagIn ++ [{tag,0,16,'IMPLICIT',32}], Bytes, OptOrMand), 
...
%%-------------------------------------------------
%% attribute number 2   External LDAP:PartialAttributeList
%%-------------------------------------------------
...
'enc_PartialAttributeList_components'([H|T],AccBytes, AccLen) ->
   {EncBytes,EncLen} = 'enc_PartialAttribute'(H, []),
...
'dec_PartialAttributeList'(Bytes, OptOrMand, TagIn) ->
   ...
   %%-------------------------------------------------
   {{_,Len},Bytes1,Rb1} = ?RT_BER:check_tags(TagIn ++ [{tag,0,16,'IMPLICIT',32}], Bytes, OptOrMand), 
   ?RT_BER:decode_components(Rb1, Len, Bytes1, fun 'dec_PartialAttribute'/3, [], []).
...
%% attribute number 2 with type SEQUENCE OF
%%-------------------------------------------------
   {EncBytes2,EncLen2} = 'enc_ModifyRequest_changes'(?RT_BER:cindex(3,Val,changes), []),
...
'enc_ModifyRequest_changes'({'ModifyRequest_changes',Val}, TagIn) ->
...
'enc_ModifyRequest_changes_components'([H|T],AccBytes, AccLen) ->
   {EncBytes,EncLen} = 'enc_ModifyRequest_changes_SEQOF'(H, []),
...
%%-------------------------------------------------
%% attribute number 2   External LDAP:PartialAttribute
%%-------------------------------------------------
   {EncBytes2,EncLen2} = 'enc_PartialAttribute'(?RT_BER:cindex(3,Val,modification), []),

   BytesSoFar = [EncBytes1, EncBytes2],
  LenSoFar = EncLen1 + EncLen2,
  ?RT_BER:encode_tags(TagIn ++ [{tag,0,16,'IMPLICIT',32}], BytesSoFar, LenSoFar).
'dec_ModifyRequest_changes_SEQOF'(Bytes, OptOrMand, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
   {{_,Len},Bytes1,Rb1} = ?RT_BER:check_tags(TagIn ++ [{tag,0,16,'IMPLICIT',32}], Bytes, OptOrMand), 
...
%% attribute number 2   External LDAP:PartialAttribute
%%-------------------------------------------------
   {Term2,Bytes4,Rb3} = 'dec_PartialAttribute'(Bytes3, mandatory, []),

   {Bytes5,Rb4} = ?RT_BER:restbytes2(RemBytes, Bytes4,noext),
   {{'ModifyRequest_changes_SEQOF', Term1, Term2}, Bytes5, Rb1+Rb2+Rb3+Rb4}.
'dec_ModifyRequest_changes'(Bytes, OptOrMand, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
   {{_,Len},Bytes1,Rb1} = ?RT_BER:check_tags(TagIn ++ [{tag,0,16,'IMPLICIT',32}], Bytes, OptOrMand), 
   ?RT_BER:decode_components(Rb1, Len, Bytes1, fun 'dec_ModifyRequest_changes_SEQOF'/3, [], []).
...
%% attribute number 2 with type SEQUENCE OF
%%-------------------------------------------------
   {Term2,Bytes4,Rb3} = 'dec_ModifyRequest_changes'(Bytes3, mandatory, []),
...
%%-------------------------------------------------
%% attribute number 2   External LDAP:AttributeList
%%-------------------------------------------------
...
%%-------------------------------------------------
%% attribute number 2   External LDAP:AttributeValueAssertion
%%-------------------------------------------------
...
%%-------------------------------------------------
   {EncBytes6,EncLen6} =  case ?RT_BER:cindex(7,Val,responseValue) of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            ?RT_BER:encode_octet_string([], ?RT_BER:cindex(7,Val,responseValue), [{tag,128,11,'IMPLICIT',32}])
       end,
...
%% attribute number 1 with type OCTET STRING OPTIONAL
%%-------------------------------------------------
   {EncBytes1,EncLen1} =  case ?RT_BER:cindex(2,Val,responseName) of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            ?RT_BER:encode_octet_string([], ?RT_BER:cindex(2,Val,responseName), [{tag,128,0,'IMPLICIT',32}])
       end,

%%-------------------------------------------------
%% attribute number 2 with type OCTET STRING OPTIONAL
%%-------------------------------------------------
   {EncBytes2,EncLen2} =  case ?RT_BER:cindex(3,Val,responseValue) of
         asn1_NOVALUE -> {<<>>,0};
         _ ->
            ?RT_BER:encode_octet_string([], ?RT_BER:cindex(3,Val,responseValue), [{tag,128,1,'IMPLICIT',32}])
       end,

   BytesSoFar = [EncBytes1, EncBytes2],
  LenSoFar = EncLen1 + EncLen2,
  ?RT_BER:encode_tags(TagIn ++ [{tag,64,25,'IMPLICIT',32},{tag,0,16,'IMPLICIT',32}], BytesSoFar, LenSoFar).


'dec_IntermediateResponse'(Bytes, OptOrMand) ->
   'dec_IntermediateResponse'(Bytes, OptOrMand, []).

'dec_IntermediateResponse'(Bytes, OptOrMand, TagIn) ->
   %%-------------------------------------------------
   %% decode tag and length 
   %%-------------------------------------------------
   {{_,Len},Bytes1,Rb1} = ?RT_BER:check_tags(TagIn ++ [{tag,64,25,'IMPLICIT',32},{tag,0,16,'IMPLICIT',32}], Bytes, OptOrMand), 
{Bytes2,RemBytes} = ?RT_BER:split_list(Bytes1,Len),
...
%%-------------------------------------------------
   {Term1,Bytes3,Rb2} = case Bytes2 of
<<2:2,_:1,0:5,_/binary>> ->
?RT_BER:decode_octet_string(Bytes2,[],[{tag,128,0,'IMPLICIT',32}], no_length, mandatory);
_ ->
{ asn1_NOVALUE, Bytes2, 0 }
end,
...
{ asn1_NOVALUE, Bytes3, 0 }
   ...
   {Bytes5,Rb4} = ?RT_BER:restbytes2(RemBytes, Bytes4,noext),
   {{'IntermediateResponse', Term1, Term2}, Bytes5, Rb1+Rb2+Rb3+Rb4}.
'maxInt'() ->
2147483647.
